...
bool studentMoveTurtle(QPointF& position, int32_t& orientation) {
    ...
    if (timer == TIMER_EXPIRED) {
        ...
        ROS_INFO("Calculated future (X1, Y1): %d, %d - (X2, Y2): %d, %d", futureX1.X, futureY1.Y,futureX2.X, futureY2.Y);
        ROS_INFO("BUMPEDFLAG: %d", bumpedFlag);
        atEnd = atend(position.x(), position.y());

        ROS_INFO("BEFORE ORIENTATION: %d", orientation);
        checkDirection(orientation, bumpedFlag, currentState);
        ROS_INFO("AFTER ORIENTATION: %d", orientation);
